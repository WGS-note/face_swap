...
def model_inference(full_frames: List[np.ndarray],
    ...
    使用原始帧，得到换面帧和转化
    """
    # 获取目标图像的Arcface emb     Get Arcface embeddings of target image
    ...
    # 从原始帧和变换中获取裁剪的面  Get the cropped faces from original frames and transformations to get those crops
    ...
    # normalize原图并emb  Normalize source images and transform to torch and get Arcface embeddings
    source_embeds = []
    ...
    for idx, (crop_frames, tfm_array, source_embed) in enumerate(zip(crop_frames_list, tfm_array_list, source_embeds)):
        ...
        # 有面的帧进行normalize   transform embeds of Xs and target frames to use by model
