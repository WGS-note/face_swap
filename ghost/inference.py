...
def main(args):
    ...
    try:
        for source_path in args.source_paths: 
            ...
            img = crop_face(img, app, args.crop_size)[0]   # 从图像中裁剪面部并调整大小
            source.append(img[:, :, ::-1])
    except TypeError:
        print("Bad source images!")
        exit()
    end_time = time.time()
    print('从原图中获取面部 Run Time: {:.0f}分 {:.0f}秒'.format((end_time - start_time) // 60, (end_time - start_time) % 60))

    # get full frames from video
    '''   按帧读取视频：视频的每一帧列表、平均帧率   '''
    start_time = time.time()
    if not args.image_to_image:
        full_frames, fps = read_video(args.target_video)   # 按帧读取视频：视频的每一帧列表、平均帧率
    else:
        target_full = cv2.imread(args.target_image)
        full_frames = [target_full]
    end_time = time.time()
    print('len full_frames:{}, fps:{}'.format(len(full_frames), fps))
    print('按帧读取视频/图片 Run Time: {:.0f}分 {:.0f}秒'.format((end_time - start_time) // 60, (end_time - start_time) % 60))

    # get target faces that are used for swap
    '''   获取需要被换脸的目标脸（视频中需要被替换的脸）   '''
    start_time = time.time()
    set_target = True
    print('List of target paths: ', args.target_faces_paths)
    if not args.target_faces_paths:
        target = get_target(full_frames, app, args.crop_size)   # 无目标脸图片
        set_target = False
    else:
        target = []
        try:
            for target_faces_path in args.target_faces_paths:   # 有目标脸图片
            ...
            exit()
    end_time = time.time()
    print('获取需要被换脸的目标脸（视频中需要被替换的脸） Run Time: {:.0f}分 {:.0f}秒'.format((end_time - start_time) // 60, (end_time - start_time) % 60))
...
if __name__ == "__main__":
    ...
    end_time = time.time()
    print('Total Time: {:.0f}分 {:.0f}秒'.format((end_time - start_time) // 60, (end_time - start_time) % 60))
